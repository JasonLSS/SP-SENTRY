...
uint8_t Infrared_Flag = 0;//模数转换大于10判定为无子弹，小于10判定为有子弹
...
{
	Infrared_Flag = Get_Adc1(ADC_Channel_3)>50;
}
...
//	u16 retry = 20000;
	...
	while((!ADC_GetFlagStatus(ADC1, ADC_FLAG_EOC ))/*&&(retry>0)*/)/*retry--*/;//等待转换结束
